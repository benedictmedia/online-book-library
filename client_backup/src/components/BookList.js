...
function BookList() {
  ...
  const [totalPages, setTotalPages] = useState(1);
    ...
    console.log("BookList component is rendering!"); // <--- ADD THIS LINE
  ...
  const handleDelete = async (bookId) => {
    ...
    try {
      ...
      if (response.status === 204) {
        ...
        // Re-fetch books to update the list or remove the deleted book from state
        if (books.length === 1 && currentPage > 1) {
          setCurrentPage(prevPage => prevPage - 1);
        } else {
          // A more robust way to refresh the list without clearing all books
          // Trigger the useEffect again by modifying one of its dependencies
          // or just filter the state if you're sure of the deletion
          setBooks(prevBooks => prevBooks.filter(book => book.id !== bookId));
          setTotalBooks(prevTotal => prevTotal - 1);
          // If the current page becomes empty after deletion and it's not the first page, go back
          if (books.filter(book => book.id !== bookId).length === 0 && currentPage > 1) {
            setCurrentPage(prevPage => prevPage - 1);
          } else {
            // Re-fetch books for the current page to ensure correct pagination counts
            // This forces a re-render and re-fetch, handling pagination and empty pages
            setBooks([]); // Temporarily clear to trigger re-fetch in useEffect based on empty books
          }
        }
      } else {
      ...
      {books.length > 0 ? (
        <>
          <ul className="book-list"> {/* Apply book-list class */}
            {books.map((book) => (
              // --- START OF MODIFICATION: Wrap the entire li in a Link ---
              <li key={book.id} className="book-item"> {/* Apply book-item class */}
                {/* Link to the book detail page */}
                <Link to={`/books/${book.id}`} style={{ textDecoration: 'none', color: 'inherit', display: 'block', height: '100%' }}>
                  <div className="book-item-content"> {/* This div was the original li content */}
                    <h3>{book.title}</h3>
                ...
                {isAdmin && (
                  <div className="book-actions"> {/* Apply book-actions class */}
                    <button onClick={() => navigate(`/edit-book/${book.id}`)} className="edit-button">Edit</button> {/* Add class */}
                    <button onClick={() => handleDelete(book.id)} className="delete-button">Delete</button> {/* Add class */}
                  </div>
                )}
              </li>
              // --- END OF MODIFICATION ---
